 ...
 /*  const { name, userId } = useUser(); // hook  */
  ...
  useEffect(() => {
    const fetchFileNames = async () => {
      try {
        const response = await axios.get("http://0.0.0.0:8000/files");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `http://0.0.0.0:8000/users/me/chat/responses`,
        {
              ...
              {messages.length > 0 ? (
                <ListGroup>
                  {messages.map((message) => {
                    ...
                    if (codeMatches) {
                      contentWithoutCode = message.content.replace(
                        codeRegex,
                        ""
                      );
                      codeBlock = (
                    ...
                    return (
                      <ListGroup.Item
                        key={message.id}
                        className={`text-${
                          message.type === "user" ? "end user" : "start bot"
                        }`}
                      >
                        <span
                          dangerouslySetInnerHTML={{
                            __html: contentWithoutCode.replace(
                              /\n\n/g,
                              "<br/>"
                            ),
                          }}
                ...
                <Form onSubmit={handleSubmit}>
                  <Row>
                      ...
                      <div className="dropdown-container">
                        <Dropdown onSelect={handleFileSelect}>
                          ...
                          >
                            ...
                            {selectedFile.display}
                          </Dropdown.Toggle>
                          <Dropdown.Menu className="dropdown-menu-custom">
                            <Dropdown.Item eventKey="All Files">
                              All Files
                            </Dropdown.Item>
                    ...
                    <Col md={3}>
                      <Button
                        type="submit"
                        className="w-100 mt-2 mt-md-0 chat-send-btn"
                      >
